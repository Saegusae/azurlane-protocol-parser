...
export function parse(data: string) {
  ...
  for (const line of lines) {
    if (importMatcher.test(line)) {
      ...
      if (name) console.debug(`Module name found: ${name}`);
      moduleName = name;
    }
  }
  ...
  for (const field in fields) {
    for (const line of lines) {
      ...
      else if (!isNaN(parseInt(val, 10))) value = parseInt(val, 10);
      else if (val === "{}") value = {};
      else if (val === "nil") value = null;
  ...
  for (const message in messages) {
    ...
    const endIndex = lines.findIndex((x) =>
      x.includes(`${message}.extensions`)
    );

    const slice = lines.slice(startIndex, endIndex);
    for (const node of slice) {
      if (node.indexOf(`${message}.`) < 0) continue;
      ...
      if (val === "true" || val === "false") value = Boolean(value);
        ...
        const subFinishIndex = slice.findIndex(
          (x, i) => i > subStartIndex && x === "}"
        );
    ...
    for (let i = 0; i < messages[message].fields.length; i++) {
      ...
      if (fields[fieldName]) messages[message].fields[i] = fields[fieldName];
    }
  }
  ...
  for (const message in messages) {
    ...
    const msgFields = messages[message].fields.sort(
      (a, b) => a.index - b.index
    );
    ...
    for (const field of msgFields) {
      msgLines.push(
        `  ${LABEL_MAP[field.label]} ${
          field.type === 11 ? field.message_type : TYPE_MAP[field.type]
